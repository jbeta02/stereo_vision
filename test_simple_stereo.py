...
# same source code as build disparity map but added x, y, z calculation and build world coord map as well
...
def get_distance_of_center(K, baseline, disparity_map):
    # get depth value at center pixel
    return find_depth(K, baseline, disparity_map[disparity_map.shape/2])


def get_distance_of_closest_obj():
    # get depth of pixel with greatest depth (will determine dist of closest obj)
    pass
...
if __name__ == "__main__":
    ...
    # print("fx, fy, ox, oy", test_calibration.get_camera_parameters(K))
    ...
    # stereo = cv2.StereoSGBM_create(numDisparities=max_disp, blockSize=11)
    # disparity = stereo.compute(img0, img1)
    # out_image5 = cv2.normalize(disparity, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)

    # stereo = cv2.StereoSGBM_create(numDisparities=max_disp, blockSize=15)
    # disparity = stereo.compute(img0, img1)
    # out_image6 = cv2.normalize(disparity, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)

    # stereo = cv2.StereoSGBM_create(numDisparities=max_disp, blockSize=25)
    # disparity = stereo.compute(img0, img1)
    # out_image7 = cv2.normalize(disparity, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)


    # stereo = cv2.StereoSGBM_create(numDisparities=max_disp, blockSize=35)
    # disparity = stereo.compute(img0, img1)
    # out_image8 = cv2.normalize(disparity, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)

    # stereo = cv2.StereoBM_create(numDisparities=max_disp, blockSize=51)
    # disparity = stereo.compute(img0, img1)
    # out_image9 = cv2.normalize(disparity, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)

    # cv2.imshow("block_size 5 ", out_image0)
    # cv2.imshow("block_size 10", out_image1)
    # cv2.imshow("block_size 25", out_image2)
    # cv2.imshow("block_size 35", out_image3)
    # cv2.imshow("block_size 50", out_image4)

    # cv2.imshow("block_size_BM 11", out_image5)
    # cv2.imshow("block_size_BM 15", out_image6)
    # cv2.imshow("block_size_BM 25", out_image7)
    # cv2.imshow("block_size_BM 35", out_image8)
    # cv2.imshow("block_size_BM 51", out_image9)
    

    stereo = cv2.StereoSGBM_create(numDisparities=64, blockSize=size)
    disparity = stereo.compute(img0, img1)
    out_image0 = cv2.normalize(disparity, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)

    stereo = cv2.StereoSGBM_create(numDisparities=128, blockSize=size)
    disparity = stereo.compute(img0, img1)
    out_image1  = cv2.normalize(disparity, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)

    # stereo = cv2.StereoSGBM_create(numDisparities=128, blockSize=size)
    # disparity = stereo.compute(img0, img1)
    # out_image2 = cv2.normalize(disparity, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)


    cv2.imshow("numDisparities_BM 32", out_image0)
    cv2.imshow("numDisparities_BM 64", out_image1)
    # cv2.imshow("numDisparities_BM 128", out_image2)
